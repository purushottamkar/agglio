...
def softplus(z, B=1):
    return np.log(1+np.exp(B*z))/B

def getObj(w, y, X, B=1):
    ip = np.dot(X,w.ravel())
    return mean_squared_error(y, softplus(ip,B))

def getGrad(w, y, X, B=1):
    r = softplus(np.dot(X,w), B)
    s = sigmoid(np.dot(X,w),B)
    n = X.shape[0]
    return (2/n)*np.dot(X.T, (r-y)*s) 

def recompute_labels(y, B):
    # recompute labels for a temperature B
    return softplus(np.log(np.exp(y)-1),B)
...
class AG_GD(BaseEstimator):
    ...
    def predict(self, X):
        return softplus(np.dot(X,self.w))
