...
def lsoftplus(z, B=1):
    return (np.log(1+np.exp(B*z))-np.log(1+np.exp(-0.2*B*z)))/B

def getObj(w, y, X, B=1):
    # lsoftplus
    ip = np.dot(X,w.ravel())
    return mean_squared_error(y, lsoftplus(ip,B))

def getGrad(w, y, X, B=1):
    # lsoftplus
    r = lsoftplus(np.dot(X,w), B)
    s1 = sigmoid(np.dot(X,w),B)
    s2 = sigmoid(np.dot(X,w),-0.2*B)
    n = X.shape[0]
    return (2/n)*np.dot(X.T, (r-y)*(s1+0.2*s2))
...
class AG_GD(BaseEstimator):
    ...
    def fit( self, X, y, w_init, w_star, max_iter = 100, minibatch_size=20, temp_cap=1):
        ...
        for i in range(max_iter):
            ...
            y_B = lsoftplus(self.ipAst, B).ravel()
    ...
    def predict(self, X):
        return lsoftplus(np.dot(X,self.w))
